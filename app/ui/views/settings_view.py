...
from ...models.stt_model import STTModel, STTLanguage, STTOutputFormat, STTDevice
...
class SettingsPage(PageBase):
    ...
    async def load(self):
        ...
        self.tab_stt = self.create_stt_settings_tab()
        self.page.on_keyboard_event = self.on_keyboard
        ...
        tabs = [
            ...
            ft.Tab(text=self._["stt_settings"], content=self.tab_stt),
        ]
    ...
    def create_stt_settings_tab(self):
        """创建STT（音频转文字）设置标签页"""
        is_mobile = self.app.is_mobile
        
        async def test_stt_model(_):
            """测试STT模型"""
            try:
                from ...core.stt_manager import STTManager
                stt_manager = STTManager(self.app)
                
                if stt_manager.is_available:
                    model_name = self.get_config_value("stt_model", "base")
                    device_info = stt_manager.get_device_info()
                    
                    if stt_manager.is_model_available(model_name):
                        device_status = ""
                        if device_info["cuda_available"]:
                            gpu_info = ", ".join([f"{d['name']} ({d['memory_gb']}GB)" for d in device_info["cuda_devices"]])
                            device_status = f"，检测到GPU: {gpu_info}"
                        
                        await self.app.snack_bar.show_snack_bar(
                            f"STT模型 {model_name} 可用{device_status}", 
                            bgcolor=ft.Colors.GREEN
                        )
                    else:
                        await self.app.snack_bar.show_snack_bar(
                            f"STT模型 {model_name} 不可用，请检查模型路径", 
                            bgcolor=ft.Colors.ORANGE
                        )
                else:
                    await self.app.snack_bar.show_snack_bar(
                        "STT依赖未安装，请运行: pip install torch faster-whisper", 
        ...
        device_options = [
            ...
            ft.dropdown.Option(key="cuda", text="CUDA (GPU)")
        ]
        ...
        try:
            ...
            if stt_manager.is_available:
                device_info = stt_manager.get_device_info()
                recommended_device = device_info["recommended_device"]
                
                # 如果检测到GPU，更新选项显示GPU信息
                if device_info["cuda_available"] and device_info["cuda_devices"]:
                    gpu_name = device_info["cuda_devices"][0]["name"]
                    device_options[1] = ft.dropdown.Option(
                        key="cuda", 
                        text=f"CUDA (GPU) - {gpu_name}"
                    )
        except Exception:
            pass
        ...
        return ft.Column(
            [
                self.create_setting_group(
                    self._["stt_basic_settings"],
                    self._["stt_basic_config"],
                    [
                        self.create_setting_row(
                            self._["stt_enabled"],
                            ft.Switch(
                                value=self.get_config_value("stt_enabled", False),
                                on_change=self.on_change,
                                data="stt_enabled",
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_auto_process"],
                            ft.Switch(
                                value=self.get_config_value("stt_auto_process", False),
                                on_change=self.on_change,
                                data="stt_auto_process",
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_model"],
                            ft.Dropdown(
                                options=[ft.dropdown.Option(model) for model in STTModel.get_models()],
                                value=self.get_config_value("stt_model", STTModel.BASE.value),
                                width=200,
                                on_change=self.on_change,
                                data="stt_model",
                                tooltip=self._["select_stt_model"],
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_language"],
                            ft.Dropdown(
                                options=[
                                    ft.dropdown.Option(key=lang, text=name) 
                                    for lang, name in STTLanguage.get_language_names().items()
                                ],
                                value=self.get_config_value("stt_language", STTLanguage.AUTO.value),
                                width=200,
                                on_change=self.on_change,
                                data="stt_language",
                                tooltip=self._["select_stt_language"],
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_output_format"],
                            ft.Dropdown(
                                options=[
                                    ft.dropdown.Option(key=fmt, text=name) 
                                    for fmt, name in STTOutputFormat.get_format_names().items()
                                ],
                                value=self.get_config_value("stt_output_format", STTOutputFormat.SRT.value),
                                width=200,
                                on_change=self.on_change,
                                data="stt_output_format",
                                tooltip=self._["select_stt_output_format"],
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_device"],
                            ft.Dropdown(
                                ...
                                value=self.get_config_value("stt_device", recommended_device),
                                width=200,
                                on_change=self.on_change,
                                data="stt_device",
                                tooltip=self._["select_stt_device"],
                            ),
                        ),
                        self.pick_folder(
                            ...
                            ft.TextField(
                                ...
                                on_change=self.on_change,
                                data="stt_model_path",
                                tooltip=self._["stt_model_path_tip"],
                            ),
                        ),
                    ],
                    is_mobile,
                ...
                self.create_setting_group(
                    self._["stt_advanced_settings"],
                    self._["stt_advanced_config"],
                    [
                        self.create_setting_row(
                            self._["stt_beam_size"],
                            ft.TextField(
                                value=str(self.get_config_value("stt_beam_size", 5)),
                                width=100,
                                keyboard_type=ft.KeyboardType.NUMBER,
                                on_change=self.on_change,
                                data="stt_beam_size",
                                tooltip=self._["stt_beam_size_tip"],
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_best_of"],
                            ft.TextField(
                                value=str(self.get_config_value("stt_best_of", 5)),
                                width=100,
                                keyboard_type=ft.KeyboardType.NUMBER,
                                on_change=self.on_change,
                                data="stt_best_of",
                                tooltip=self._["stt_best_of_tip"],
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_temperature"],
                            ft.TextField(
                                value=str(self.get_config_value("stt_temperature", 0)),
                                width=100,
                                keyboard_type=ft.KeyboardType.NUMBER,
                                on_change=self.on_change,
                                data="stt_temperature",
                                tooltip=self._["stt_temperature_tip"],
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_vad_filter"],
                            ft.Switch(
                                value=self.get_config_value("stt_vad_filter", True),
                                on_change=self.on_change,
                                data="stt_vad_filter",
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_condition_on_previous_text"],
                            ft.Switch(
                                value=self.get_config_value("stt_condition_on_previous_text", False),
                                on_change=self.on_change,
                                data="stt_condition_on_previous_text",
                            ),
                        ),
                        self.create_setting_row(
                            self._["stt_initial_prompt_zh"],
                            ft.TextField(
                                value=self.get_config_value("stt_initial_prompt_zh", "转录为中文简体。"),
                                width=300,
                                on_change=self.on_change,
                                data="stt_initial_prompt_zh",
                                tooltip=self._["stt_initial_prompt_zh_tip"],
                            ),
                        ),
                        self.create_setting_row(
                            self._["test_stt_model"],
                            ft.ElevatedButton(
                                text=self._["test_model"],
                                icon=ft.Icons.PLAY_ARROW,
                                on_click=test_stt_model,
                                tooltip=self._["test_model_tip"],
                            ),
                        ),
                    ],
                    is_mobile,
