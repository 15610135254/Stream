...
class RecordingDialog:
    ...
    async def show_dialog(self):
        ...
        async def on_preview_click(_):
            """打开预览对话框"""
            url = url_field.value.strip()
            if not url:
                await self.app.snack_bar.show_snack_bar("请先输入直播间URL", bgcolor=ft.colors.RED)
                return
            
            quality = quality_dropdown.value
            preview_dialog = PreviewDialog(self.app, url=url, quality=quality)
            preview_dialog.open = True
            self.app.dialog_area.content = preview_dialog
