...
class LiveStreamRecorder:
    ...
    async def start_ffmpeg(
        ...
        try:
            ...
            if return_code in safe_return_code:
                ...
                if self.user_config.get("stt_enabled") and self.user_config.get("stt_auto_process"):
                    logger.info("Preparing STT processing in the background")
                    try:
                        self.app.page.run_task(
                            self.process_stt_after_recording,
                            save_file_path,
                            self.segment_record
                        )
                        logger.success("Successfully added STT processing task")
                    except Exception as e:
                        logger.error(f"Failed to process STT: {e}")
                        await self.process_stt_after_recording(save_file_path, self.segment_record)
    ...
    async def process_stt_after_recording(self, recording_path: str, is_segment_record: bool):
        """录制完成后处理STT转录"""
        try:
            ...
            if not stt_manager.is_available:
                logger.warning("STT功能不可用，跳过音频转文字处理")
                return
            
            config = stt_manager.get_stt_config()
            if not config["enabled"]:
                return
            
            files_to_process = []
            
            if is_segment_record:
                recording_dir = os.path.dirname(recording_path)
                file_base = os.path.basename(recording_path).rsplit("_", maxsplit=1)[0]
                
                for file_path in utils.get_file_paths(recording_dir):
                    if file_base in file_path and os.path.isfile(file_path):
                        files_to_process.append(file_path)
            else:
                if os.path.exists(recording_path):
                    files_to_process.append(recording_path)
            
            for file_path in files_to_process:
                if not self.app.recording_enabled:
                    logger.info(f"应用正在关闭，将STT任务添加到后台服务: {file_path}")
                    BackgroundService.get_instance().add_task(
                        stt_manager.process_recording_sync, file_path
                    )
                else:
                    logger.info(f"开始处理STT转录: {file_path}")
                    result = await stt_manager.process_recording_file(file_path)
                    
                    if result["success"]:
                        logger.success(f"STT转录完成: {result.get('output_path', file_path)}")
                    else:
                        logger.error(f"STT转录失败: {result.get('error', '未知错误')}")
                        
        except ImportError:
            logger.warning("STT依赖未安装，跳过音频转文字处理")
        except Exception as e:
            logger.error(f"STT处理出现异常: {e}")
