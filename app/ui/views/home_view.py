...
from ..components.batch_operations_dialog import BatchOperationsDialog
...
class HomePage(PageBase):
    ...
    async def load(self):
        ...
        await self.update_batch_buttons_state()
    ...
    def create_batch_start_button(self):
        """创建批量开始按钮"""
        self.batch_start_button = ft.IconButton(
            icon=ft.Icons.PLAY_ARROW,
            tooltip=self._["batch_start"],
            on_click=self.start_monitor_recordings_on_click,
            disabled=True  # 初始状态为禁用
        )
        return self.batch_start_button

    def create_batch_stop_button(self):
        """创建批量停止按钮"""
        self.batch_stop_button = ft.IconButton(
            icon=ft.Icons.STOP,
            tooltip=self._["batch_stop"],
            on_click=self.stop_monitor_recordings_on_click,
            disabled=True  # 初始状态为禁用
        )
        return self.batch_stop_button

    def create_batch_delete_button(self):
        """创建批量删除按钮"""
        self.batch_delete_button = ft.IconButton(
            icon=ft.Icons.DELETE_SWEEP,
            tooltip=self._["batch_delete"],
            on_click=self.delete_monitor_recordings_on_click,
            disabled=True  # 初始状态为禁用
        )
        return self.batch_delete_button

    async def update_batch_buttons_state(self):
        """更新批量操作按钮状态"""
        has_recordings = len(self.app.record_manager.recordings) > 0
        selected_recordings = await self.app.record_manager.get_selected_recordings()
        has_selected = len(selected_recordings) > 0
        
        # 批量开始/停止按钮：有录制项目时启用
        if self.batch_start_button:
            self.batch_start_button.disabled = not has_recordings
            self.batch_start_button.update()
            
        if self.batch_stop_button:
            self.batch_stop_button.disabled = not has_recordings
            self.batch_stop_button.update()
            
        # 批量删除按钮：有录制项目时启用（选中优先，没选中则全部删除）
        if self.batch_delete_button:
            self.batch_delete_button.disabled = not has_recordings
            self.batch_delete_button.update()
    ...
    def create_home_title_area(self):
        ...
        batch_controls = [
            ...
            self.create_batch_start_button(),
            self.create_batch_stop_button(),
            self.create_batch_delete_button(),
            ft.IconButton(
                icon=ft.Icons.SETTINGS,
                tooltip="批量操作",
                on_click=self.batch_operations_on_click,
            ),
        ]
    ...
    async def batch_operations_on_click(self, e):
        """打开批量操作对话框"""
        selected_recordings = await self.app.record_manager.get_selected_recordings()
        
        batch_dialog = BatchOperationsDialog(self.app, selected_recordings)
        batch_dialog.open = True
        self.app.dialog_area.content = batch_dialog
        self.app.dialog_area.update()
